    ...
    <div class="row">
        ...
        <div class="col-md-3">
            <?= $form->field($model, 'id_estado')
                ->dropDownList(ArrayHelper::map(CatEstados::find()
                ->orderBy('txt_nombre')
                ->asArray()->all(), 'id_estado', 'txt_nombre'),['prompt' => 'Seleccionar estado', 'class' => 'panel-search-form-select'])
                ->label(false) ?>
        </div>
        ...
        <div class="col-md-3">
            <div class="form-group form-group-btns">
                <?= Html::submitButton('<i class="icon wb-plus"></i> Search', ['class' => 'btn btn-search']) ?>
                <!-- <?= Html::resetButton('Reset', ['class' => 'btn btn-clear']) ?> -->
            </div>
        </div>
